...
uniform Sphere u_sphairahedronSpheres[{{ numSphairahedronSpheres }}];
...
float DistSphairahedron(vec3 pos) {
	...
	d = max(-DistSphere(pos, u_excavationSpheres[{{ n }}]),
	 		d);
	{% endfor %}
	{% for n in range(0, numSphairahedronSpheres) %}
     d = max(-DistSphere(pos, u_sphairahedronSpheres[{{ n }}]),
	 		d);
	{% endfor %}
    return d;
}
...
float DistLimitsetFromSphairahedron(vec3 pos, out float invNum) {
    float dr = 1.;
    invNum = 0.;
    for(int i = 0; i < 1000; i++) {
        ...
        bool inFund = true;
		{% for n in range(0, numSphairahedronSpheres) %}
		if(distance(pos, u_sphairahedronSpheres[{{ n }}].center) < u_sphairahedronSpheres[{{ n }}].r.x) {
            invNum += (float({{ (n + 1) *  10 }}) + invNum) * u_colorWeight + 1.;
			SphereInvert(pos, dr,
						 u_sphairahedronSpheres[{{ n }}].center,
						 u_sphairahedronSpheres[{{ n }}].r);
            continue;
		}
		{% endfor %}
        if(inFund) break;
...
vec4 distFunc(const vec3 pos) {
    ...
    hit = DistUnion(hit,
                    vec4(DistLimitsetFromSphairahedron(pos, g_invNum), ID_PRISM, -1, -1));
    return hit;
}
...
vec4 computeColor(const vec3 rayOrg, const vec3 rayDir) {
    ...
    for(int depth = 0 ; depth < 8; depth++){
        ...
        hit = IntersectBoundingSphere(u_boundingSphere.center,
